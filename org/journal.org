#+TAGS: noexport(n) software(s) Alexis(a) Florence(f) deprecated(d) EM(e) HMM(h) MMST(m) R(r) C(c) Bug(b) Optimization(o) Profiling(P) Performance Meeting
...
***** TODO Modify R call to C++
      Changes:
      - 
      =Data_importation_v2016_02_02.R= retrieve information passed to
      the R script. It reads csv files and concatenate then if there
      are several and reindex the data. It also retrieves
...
#+end_example
     ...
     Data was not load correctly. The index to load the first row of the submatrix was incorrect.
